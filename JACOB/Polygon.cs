...
namespace JACOB
{
    class Polygon : Shape
    {
        ...
        public Polygon(Vector2 p, double circumference, int sides, string shapename)
        {

            this.Shapename = shapename;
            this.circumference = circumference;
            this.sides = sides;
            Verticies = CalculateVerticies(p.X, p.Y, circumference, sides);
        ...
        public Vector2[] CalculateVerticies(double x, double y, double circumference, int sides)
        {
            double offset;
            double sidelength = circumference / sides;
            double circumradius = (sidelength / 2) * Math.Cos(Math.PI / sides) / (Math.Sin(Math.PI / sides));
            Vector2 centrepoint = new Vector2(x, y);
